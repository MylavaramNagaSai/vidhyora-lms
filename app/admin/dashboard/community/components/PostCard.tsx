...
export default function PostCard({ post, currentUser, onAction }: { post: Post, currentUser: CurrentUser, onAction: (action: string, post: Post, newContent?: string) => void }) {
  ...
  // NEW: Closes all popups instantly when clicking outside
  const closeAllMenus = () => {
    setShowMenu(false);
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(shareLink).then(() => {
      setIsCopied(true);
      setTimeout(() => { setIsCopied(false); closeAllMenus(); }, 2000);
    });
  };

  // NEW: Smooth scroll/focus to the comment input when "Comment" is clicked
  ...
  return (
    <div className={`bg-white border rounded-3xl p-5 shadow-sm transition-all relative z-0 ${post.isPinned ? 'border-blue-300 shadow-blue-500/10' : 'border-slate-200 hover:shadow-md'}`}>
      
      {/* 
        INVISIBLE OVERLAY: 
        This catches clicks anywhere else on the screen and closes the popups! 
      */}
      {(showMenu || showShareMenu || showReactionList) && (
        <div className="fixed inset-0 z-10 cursor-default" onClick={(e) => { e.stopPropagation(); closeAllMenus(); }} />
      )}

      {post.isPinned && <div className="flex items-center gap-1.5 text-xs font-black text-blue-600 uppercase tracking-widest mb-4 ml-12"><PinIcon /> Pinned Post</div>}

      <div className="flex items-start justify-between mb-4 relative z-20 pointer-events-none">
        
        {/* NEW: Repost Header UI */}
        {post.isRepost ? (
          <div className="flex items-start gap-3 pointer-events-auto">
            <div className="w-10 h-10 rounded-full flex items-center justify-center font-black text-white shadow-sm bg-slate-800 shrink-0">
              {post.originalAuthor?.charAt(0) || 'O'}
            </div>
            <div>
              <h4 className="font-bold text-slate-900 leading-none">This content is posted by {post.originalAuthor}</h4>
              <div className="mt-1.5 border-l-[3px] border-slate-200 pl-2.5">
                <span className="text-xs font-bold text-slate-500 block">Reposted by {post.authorName}</span>
                <span className="text-[10px] font-medium text-slate-400 mt-0.5 block">{timeStr}</span>
              </div>
            </div>
          </div>
        ) : (
          /* Original Post Header UI */
          <div className="flex items-center gap-3 pointer-events-auto">
            ...
            <div>
              ...
              <span className="text-xs font-medium text-slate-400">
                {post.scheduledFor ? 'Scheduled Post' : <>{timeStr} {post.isEdited && <span className="italic font-bold text-slate-500 ml-1">(Edited)</span>}</>}
              </span>
            </div>
          </div>
        )}
        ...
        {(isAuthor || isAdmin) && (
          <div className="relative pointer-events-auto">
            ...
            {showMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 shadow-xl rounded-2xl py-2 animate-in fade-in zoom-in-95">
                {isAdmin && <button onClick={() => { onAction('pin', post); setShowMenu(false); }} className="w-full text-left px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">{post.isPinned ? 'Unpin Post' : 'Pin to Top'}</button>}
                <button onClick={() => { onAction('delete', post); setShowMenu(false); }} className="w-full text-left px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-50">Delete Post</button>
              </div>
            )}
          </div>
        )}
      ...
      <div className="mb-4">
        ...
        {post.audioUrl && <div className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 mb-4 flex flex-col gap-2"><span className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Voice Note / Audio File</span><audio src={post.audioUrl} controls className="w-full h-10 outline-none" /></div>}
      </div>

      {/* Reaction List Popup */}
      {reactionCount > 0 && (
        <div className="relative mb-3 z-20 inline-block">
          ...
          {showReactionList && (
            <div className="absolute left-0 bottom-full mb-2 w-64 bg-white border border-slate-100 shadow-2xl rounded-2xl py-3 px-4 animate-in fade-in zoom-in-95 slide-in-from-bottom-2">
      ...
      {/* Main Action Bar */}
      <div className="flex items-center justify-between border-t border-slate-100 pt-3 relative z-20">
        <div className="flex gap-1 w-full">
          <ReactionButton post={post} currentUser={currentUser} />
          
          {/* Clicking this now focuses the permanent comment input below! */}
          <button onClick={handleCommentFocus} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl transition-colors text-xs font-bold hover:bg-slate-50 text-slate-500">
            <MessageIcon /> Comment
          </button>
          ...
          <div className="flex-1 relative">
            ...
            {showShareMenu && (
              <div className="absolute bottom-full right-0 mb-2 w-56 bg-white border border-slate-100 shadow-2xl rounded-2xl p-2 animate-in fade-in zoom-in-95 slide-in-from-bottom-2">
      ...
      {/* NEW: Unconditionally rendered permanent Comment Section */}
